...
#include "aclnn_get_weight_by_rank.h"
#include "common.h"
...
bool OpRunner::RunOp()
{
    ...
    INFO_LOG("Execute aclnnGetWeightByRankGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnGetWeightByRank(workspace_, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyEvent(startEvent);
        (void)aclrtDestroyEvent(endEvent);
        (void)aclDestroyIntArray(lensArray);
    ...
    INFO_LOG("Execute aclnnGetWeightByRank success");
